...
import com.wei.system.service.ISysPostService;
...
public class SysUserController extends BaseController {
    ...
    @ApiOperation(value = "根据角色关键字获取用户下拉列表", notes = "......")
    @RequestMapping(value = "/system/user/role-user/drop-down-list", method = GET)
    @ResponseBody
    public Response<Integer, PageResponseVo<UserDropDownVo>> getUserDropDownListByRoleKey(
            @RequestParam(value = "roleKey", required = true) String roleKey
            , @RequestParam(value = "keywords", required = false) String keywords
            , @RequestParam(value = "pageNo", required = false, defaultValue = "1") int pageNo
            , @RequestParam(value = "pageSize", required = false, defaultValue = "10") int pageSize) {
        int size = userService.queryUserDropDownCountByRoleKey(roleKey, keywords);
        List<UserDropDownVo> list = userService.queryUserDropDownListByRoleKey(roleKey, keywords, pageNo, pageSize);

        PageResponseVo<UserDropDownVo> pageVo = PageResponseVo.<UserDropDownVo>builder()
                .size(size)
                .list(list)
                .build();
