...
public class SysProfileController extends BaseController {
     ...
     * 个人信息
     */
    @RequestMapping(value = "/system/user/profile", method = GET)
    @ResponseBody
    ...
    public AjaxResult avatar(@RequestParam("file") MultipartFile file) throws IOException {
        ...
        Response<Boolean, FileVo> resp = fileService.upload(file.getBytes(), null, originFileName, fileName);
        if (!resp.getCode()) {
            return AjaxResult.error("上传图片异常，请联系管理员[" + resp.getMsg() + "]");
        }
        String avatar = null;
        if (resp.getData() == null || (avatar = resp.getData().getUrl2()) == null) {
            return AjaxResult.error("上传图片异常，请联系管理员");
        }
