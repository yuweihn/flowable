...
import com.wei.system.service.ISysDeptService;
...
public class SysDeptController extends BaseController
{
    ...
    public AjaxResult add(@Validated @RequestBody SysDept dept) {
        ...
        SysUser sysUser = sysUserService.selectUserById(leaderId);
        if (sysUser == null) {
            return AjaxResult.error("新增部门'" + dept.getDeptName() + "'失败，用户不存在");
        }

        if (UserConstants.NOT_UNIQUE.equals(deptService.checkDeptNameUnique(dept))) {
            return AjaxResult.error("新增部门'" + dept.getDeptName() + "'失败，部门名称已存在");
        }
        dept.setLeader(sysUser.getNickName());
        dept.setPhone(sysUser.getPhonenumber());
    ...
    {
        SysUser sysUser = sysUserService.selectUserById(dept.getLeaderId());
        if (sysUser == null){
            return AjaxResult.error("修改部门'" + dept.getDeptName() + "'失败，用户不存在");
        }
        if (UserConstants.NOT_UNIQUE.equals(deptService.checkDeptNameUnique(dept)))
        {
