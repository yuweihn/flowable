...
public class FindNextNodeUtil {
     ...
     * @return
     */
    public static List<UserTask> getNextUserTasks(RepositoryService repositoryService, org.flowable.task.api.Task curTask, Map<String, Object> map,boolean flag) {
        List<UserTask> data = new ArrayList<>();
        ProcessDefinition processDefinition = repositoryService.createProcessDefinitionQuery().processDefinitionId(curTask.getProcessDefinitionId()).singleResult();
        BpmnModel bpmnModel = repositoryService.getBpmnModel(processDefinition.getId());
        Process mainProcess = bpmnModel.getMainProcess();
        Collection<FlowElement> flowElements = mainProcess.getFlowElements();
        String key = curTask.getTaskDefinitionKey();
        FlowElement curFlowElement = bpmnModel.getFlowElement(key);
        next(flowElements, curFlowElement, map, data,flag);
        return data;
    }

    public static void next(Collection<FlowElement> flowElements, FlowElement curFlowElement, Map<String, Object> map, List<UserTask> nextUser,boolean flag) {
        //如果是结束节点
        if (curFlowElement instanceof EndEvent) {
            //如果是子任务的结束节点
            if (getSubProcess(flowElements, curFlowElement) != null) {
                curFlowElement = getSubProcess(flowElements, curFlowElement);
            }
        }
        //获取Task的出线信息--可以拥有多个
        List<SequenceFlow> outGoingFlows = null;
        if (curFlowElement instanceof Task) {
            outGoingFlows = ((Task) curFlowElement).getOutgoingFlows();
        } else if (curFlowElement instanceof Gateway) {
            outGoingFlows = ((Gateway) curFlowElement).getOutgoingFlows();
        } else if (curFlowElement instanceof StartEvent) {
            outGoingFlows = ((StartEvent) curFlowElement).getOutgoingFlows();
        } else if (curFlowElement instanceof SubProcess) {
            outGoingFlows = ((SubProcess) curFlowElement).getOutgoingFlows();
        } else if (curFlowElement instanceof CallActivity) {
            outGoingFlows = ((CallActivity) curFlowElement).getOutgoingFlows();
        }
            ...
            for (SequenceFlow sequenceFlow : outGoingFlows) {
                ...
                if (expression == null ||
                    ...
                    if (nextFlowElement instanceof CallActivity) {
                        ...
                        next(flowElements, nextFlowElement, map, nextUser, flag);
                    }
                    //用户任务
